import unittest
from utas_login import *
"""
Tests to run:
- loading the date from config file
- converting the data
- executing commands by extending class 
  and adding stubs in place of driver running
"""
...
class utas_login(unittest.TestCase):
    ...
    def testEnvironmentLoading(self):
        self.assertEqual(self.env.usrName,'Samuel.geale@utas.edu.au')
        self.assertEqual(self.env.usrPass,'17SG@rxp30')
        self.assertEqual(self.env.portalURL,'https://utastest.service-now.com/selfservice?id=home')
        self.assertEqual(self.env.ITIL_URL,'https://utastest.service-now.com/')
